...
fun ErrorDomain?.toStateApiError(): State.Error = when (this) {
    null -> State.Error.ConnectionError
    else -> State.Error.ApiError(
        code,
        when (this) {
            is ErrorDomain.BadRequest -> this.message
            is ErrorDomain.AccessDenied -> this.message
